...
class MainWindow(QMainWindow):
    ...
    def post_main_widget(self) -> None:
        ...
        self.speed_button.clicked.connect(lambda: post_change_speed(self))
    ...
    def keyPressEvent(self, event: QEvent) -> None:
        ...
        if event.key() == Qt.Key.Key_Return or event.key() == Qt.Key.Key_Enter:
            ...
            if self.main_shown:
                ...
                submenu_shown: bool = self.submenu_shown
                if hasattr(self, "worker") and self.worker.isRunning():
                    self.worker.stop()
                    self.worker.wait()
                if submenu_shown:
                    for button in self.main_menu_buttons:
                        if button["name"] == self.last_clicked_button:
                            QApplication.processEvents()
                            QTimer.singleShot(0, lambda: button["function"](self, button["name"]))
                            break
                    self.submenu_shown = False
                else:
                    self.post_main_widget()
